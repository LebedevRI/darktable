...
static void _set_hinter_message(dt_masks_form_gui_t *gui, dt_masks_type_t formtype)
{
  ...
  {
    if (gui->creation) strncat(msg,_("ctrl+click to add a sharp node"),sizeof(msg)-strlen(msg)-1);
    else if (gui->point_selected >= 0) strncat(msg,_("ctrl+click to switch between smooth/sharp node"),sizeof(msg)-strlen(msg)-1);
    else if (gui->feather_selected >= 0) strncat(msg,_("right-click to reset feather value"),sizeof(msg)-strlen(msg)-1);
    else if (gui->seg_selected >= 0) strncat(msg,_("ctrl+click to add a node"),sizeof(msg)-strlen(msg)-1);
    else if (gui->form_selected) strncat(msg,_("ctrl+scroll to set shape opacity"),sizeof(msg)-strlen(msg)-1);
  }
  else if (formtype&DT_MASKS_GRADIENT)
  {
    if (gui->form_selected) strncat(msg,_("ctrl+scroll to set shape opacity"),sizeof(msg)-strlen(msg)-1);
    else if (gui->pivot_selected) strncat(msg,_("move to rotate shape"),sizeof(msg)-strlen(msg)-1);
  }
  else if (formtype&DT_MASKS_ELLIPSE)
  {
    if (gui->point_selected >= 0) strncat(msg,_("ctrl+click to rotate"),sizeof(msg)-strlen(msg)-1);
    else if (gui->form_selected) strncat(msg,_("ctrl+scroll to set shape opacity"),sizeof(msg)-strlen(msg)-1);
  }
  else if (formtype&DT_MASKS_BRUSH)
  {
    if (gui->creation) strncat(msg,_("scroll to set brush size, shift+scroll to set hardness, ctrl+scroll to set opacity"),sizeof(msg)-strlen(msg)-1);
    else if (gui->border_selected) strncat(msg,_("scroll to set brush size"),sizeof(msg)-strlen(msg)-1);
    else if (gui->form_selected) strncat(msg,_("scroll to set hardness, ctrl+scroll to set shape opacity"),sizeof(msg)-strlen(msg)-1);
  }
...
int dt_masks_get_points_border(dt_develop_t *dev, dt_masks_form_t *form, float **points, int *points_count, float **border, int *border_count, int source)
{
  ...
  else if (form->type & DT_MASKS_GRADIENT)
  {
    ...
    if (dt_gradient_get_points(dev, gradient->anchor[0], gradient->anchor[1], gradient->rotation, points, points_count))
    {
      if (border) return dt_gradient_get_points_border(dev, gradient->anchor[0], gradient->anchor[1], gradient->rotation, gradient->compression, border, border_count);
      else return 1;
    }
  }
...
int dt_masks_events_button_released (struct dt_iop_module_t *module, double x, double y, int which, uint32_t state)
{
  ...
  else if (form->type & DT_MASKS_BRUSH) return dt_brush_events_button_released(module,pzx,pzy,which,state,form,0,gui,0);
...
int dt_masks_events_mouse_scrolled (struct dt_iop_module_t *module, double x, double y, int up, uint32_t state)
{
  ...
  else if (form->type & DT_MASKS_BRUSH) return dt_brush_events_mouse_scrolled(module,pzx,pzy,up,state,form,0,gui,0);
...
void dt_masks_events_post_expose (struct dt_iop_module_t *module, cairo_t *cr, int32_t width, int32_t height, int32_t pointerx, int32_t pointery)
{
  ...
  else if (form->type & DT_MASKS_BRUSH) dt_brush_events_post_expose(cr,zoom_scale,gui,0,g_list_length(form->points));
  cairo_restore(cr);
}
...
void dt_masks_clear_form_gui(dt_develop_t *dev)
{
  ...
  dev->form_gui->form_selected = dev->form_gui->border_selected = dev->form_gui->form_dragging = dev->form_gui->form_rotating = FALSE;
...
void dt_masks_iop_combo_populate(struct dt_iop_module_t **m)
{
  ...
  while (forms)
  {
    ...
    if (!used)
    {
      ...
      {
        char str2[256] = "<";
        strncat(str2,_("add existing shape"),sizeof(str2)-strlen(str2)-1);
  ...
  while (modules)
  {
    ...
    if ((m!=module) && (m->flags() & IOP_FLAGS_SUPPORTS_BLENDING) && !(m->flags() & IOP_FLAGS_NO_MASKS))
    {
      ...
      if (grp)
      {
        ...
        {
          char str2[256] = "<";
          strncat(str2,_("use same shapes as"),sizeof(str2)-strlen(str2)-1);
          dt_bauhaus_combobox_add(combo,str2);
          cids[pos++] = 0;  //nothing to do
        }
        char str[256] = "";
        strncat(str,m->name(),sizeof(str)-strlen(str)-1);
        strcat(str," ");
