...
int dt_opencl_lock_device(const int pipetype)
{
  ...
  if(priority)
  {
    ...
    for(int n = 0; n < nloop; n++)
    {
      ...
      while(*prio != -1)
      {
        if(!dt_pthread_mutex_BAD_trylock(&cl->dev[*prio].lock.Mutex))
        {
          int devid = *prio;
          free(priority);
          return devid;
  ...
  else
  {
    ...
    for(int try_dev = 0; try_dev < cl->num_devs; try_dev++)
    {
      ...
      if(!dt_pthread_mutex_BAD_trylock(&cl->dev[try_dev].lock.Mutex)) return try_dev;
    }
  }
...
void dt_opencl_unlock_device(const int dev)
{
  ...
  dt_pthread_mutex_BAD_unlock(&cl->dev[dev].lock.Mutex);
}
